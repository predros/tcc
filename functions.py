...
def Stiffness(length, elasticity, inertia, area, axial):
    ...
    for i in range(len(length)):
        ...
        ST.append([[E*A/L, 0, 0, -E*A/L, 0, 0],
                [0, 2*E*I*(C+S)/(L*L*L)+P/L, E*I*(C+S)/(L*L), 0, -2*E*I*(C+S)/(L*L*L)-P/L, E*I*(C+S)/(L*L)],
                [0, E*I*(C+S)/(L*L), E*I*C/L, 0, -E*I*(C+S)/(L*L), E*I*S/L],
                [-E*A/L, 0, 0, E*A/L, 0, 0],
                [0, -2*E*I*(C+S)/(L*L*L)-P/L, -E*I*(C+S)/(L*L), 0, 2*E*I*(C+S)/(L*L*L)+P/L, -E*I*(C+S)/(L*L)],
                [0, E*I*(C+S)/(L*L), E*I*S/L, 0, -E*I*(C+S)/(L*L), E*I*C/L]])
    return ST

def LoadVector(length, elasticity, inertia, area, thermal, Qy, Qx, Tinf, Tsup, yinf, ysup, displace, curvature):
    F0 = []
    F0q = []
    F0t = []
    F0s = []
    ...
    for i in range(len(length)):
        ...
        qy = Qy[i]
        qx = Qx[i]
        F0q.append([-qx*L/2, qy*L/2, qy*L*L/12, -qx*L/2, qy*L/2, -qy*L*L/12])
    # Thermal load
    for i in range(len(length)):
        L = length[i]
        T1 = Tinf[i]
        ...
        alpha = thermal[i]
        T0 = (T1-T2)*y2/(y1+y2)
        F0t.append([E*A*alpha*T0, 0, alpha*E*I*(T1-T2)/h, -E*A*alpha*T0, 0, -alpha*E*I*(T1-T2)/h])
    ...
    for i in range(len(length)):
        ...
        A = area[i]
        I = inertia[i]
        F0s.append([E*A*delta/L, 0, 8*E*I*e/(L*L), -E*A*delta/L, 0, -8*E*I*e/(L*L)])
    # Final vector
    F0 = np.add(F0q, F0t)
    F0 = np.add(F0, F0s)
    return F0
...
# Calculates the degrees of freedom matrix, given the number of node displacements and restrictions.
...
# Defines the degrees of freedom relative to each frame's start and end nodes.
def FrameDOF(START, END, DOF):
    FD = []
    for i in range(0, len(START)):
        a = 3*START[i]
        b = 3*END[i]
        FD.append([DOF[a], DOF[a+1], DOF[a+2], DOF[b], DOF[b+1], DOF[b+2]])
    return FD
...
# Defines the prescribed displacement relative to each frame's start and end nodes.
def FramePD(START, END, PD):
    FramePD = []
    for i in range(0, len(START)):
        a = 3*START[i]
        b = 3*END[i]
        FramePD.append([PD[a], PD[a+1], PD[a+2], PD[b], PD[b+1], PD[b+2]])
    return FramePD
...
# Divides the nodal displacement matrix into three matrices, relative to the three axes.
...
# Run the structural analysis calculations using Galambos' method, given an arbitrary vector P.
def G_Run(N_X, N_Y, N_PX, N_PY, N_MZ, N_RESTRX, N_RESTRY, N_RESTRZ, N_KX, N_KY, N_KZ, N_PDX, N_PDY, N_PDZ, FR_START, FR_END, FR_Qx, FR_Qy, FR_Tinf, FR_Tsup, FR_yinf, FR_ysup, FR_tensile, FR_curvature, FR_E, FR_I, FR_A, FR_alpha, P):
    # Basic parameters
    nnode = len(N_X)
    nframe = len(FR_START)
    nd = 3*len(N_X)
    ...
    FR_F0L = LoadVector(FR_L, FR_E, FR_I, FR_A, FR_alpha, FR_Qy, FR_Qx, FR_Tinf, FR_Tsup, FR_yinf, FR_ysup, FR_tensile, FR_curvature)
...
def G_Iteration(N_X, N_Y, N_PX, N_PY, N_MZ, N_RESTRX, N_RESTRY, N_RESTRZ, N_KX, N_KY, N_KZ, N_PDX, N_PDY, N_PDZ, FR_START, FR_END, FR_Qx, FR_Qy, FR_Tinf, FR_Tsup, FR_yinf, FR_ysup, FR_tensile, FR_curvature, FR_E, FR_I, FR_A, FR_alpha, maxerror, maxiter):
    P1 = np.zeros(len(FR_START))
    P2 = np.zeros(len(FR_START))
    iterations = 0
    while 1 == 1:
        X = G_Run(N_X, N_Y, N_PX, N_PY, N_MZ, N_RESTRX, N_RESTRY, N_RESTRZ, N_KX, N_KY, N_KZ, N_PDX, N_PDY, N_PDZ, FR_START, FR_END, FR_Qx, FR_Qy, FR_Tinf, FR_Tsup, FR_yinf, FR_ysup, FR_tensile, FR_curvature, FR_E, FR_I, FR_A, FR_alpha, P1)
        err_vector = []
        ...
        if iterations >= maxiter or error <= maxerror: break
...
N_RESTRX = [1, 0, 0, 1] # Nodal X-direction displacement restriction (1 = restricted, 0 = unknown)
N_RESTRY = [1, 0, 0, 1] # Nodal Y-direction displacement restriction (1 = restricted, 0 = unknown)
N_RESTRZ = [1, 0, 0, 1] # Nodal Z-direction rotation restriction (1 = restricted, 0 = unknown)
N_KX = [0, 0, 0, 0] # Nodal X-direction spring stiffness (0 = no spring present)
N_KY = [0, 0, 0, 0] # Nodal Y-direction spring stiffness (0 = no spring present)
N_KZ = [0, 0, 0, 0] # Nodal Z-direction spring stiffness (0 = no spring present)
N_PDX = [0, 0, 0, 0] # Nodal X-direction prescribed displacement (only usable when RESTRX[i] = 1)
N_PDY = [0, 0, 0, 0] # Nodal Y-direction prescribed displacement (only usable when RESTRY[i] = 1)
N_PDZ = [0, 0, 0, 0] # Nodal Z-direction prescribed rotation (only usable when RESTRZ[i] = 1)
...
FR_alpha = [1/1000000, 1/1000000, 1/1000000] # Thermal coefficient for each frame
...
FR_yinf = [7.5, 7.5, 7.5] # y-coordinate (absolute value) of the inferior side of each frame
FR_ysup = [7.5, 7.5, 7.5] # y-coordinate (absolute value) of the superior side of each frame
FR_tensile = [0, 0, 0] # Tensile pre-displacement of each frame
FR_curvature = [0, 0, 0] # Central initial displacement of each frame (circular curvature)
P = [0, 0, 0]
